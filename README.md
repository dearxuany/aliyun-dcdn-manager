...
* Jenkins CD 触发 K8S 部署完毕后，有以下两个以脚本形式执行操作：
** 发送请求到阿里云 DCDN 刷新 API 以目录或 URL 形式刷新 DCDN
** 转换新版本前端文件列表为在线可请求的 URL 格式列表，生产者发送到 kafka 集群待预热
* Kafka 消费者服务间隔查询阿里云 DCDN 任务队列，当无任务执行时，消费 URL 列表消息并发送阿里云 DCDN API 预热请求
## 项目描述
