# aliyun-dcdn-manager
阿里云全站加速 DCDN 管理服务，主要用于前端应用版本更新所需的全站加速 DCDN 刷新、预热自动化处理，由 jenkins 于 CICD 完成 k8s 新版本代码部署发布时触发。

主要包含以下几个部分：
- Jenkins CI 打包过程生成版本前端文件列表
- Jenkins CD 完成新版应用部署到 K8S 后触发 DCDN 刷新脚本
  - 发送请求到阿里云 DCDN 刷新 API 以目录或 URL 形式刷新 DCDN
  - 转换新版本前端文件列表为在线可请求的 URL 格式列表，生产者发送 URL 列表消息到 kafka 集群待预热
- Kafka 消费者 DCDN 预热服务
  - 间隔查询阿里云 DCDN 任务队列，未完成时等待
  - 当无任务执行时，消费 URL 列表消息并发送预热请求到阿里云 DCDN API，限速对新版本前端文件进行预热
